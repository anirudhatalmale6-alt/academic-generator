...
class AcademicDocBuilder:
    ...
    def add_cover_page(
        ...
        # Supervisor and student (left-right alignment via tab)
    ...
    # ─── Table of Contents (Word-native field code) ────────────────────

    def add_toc(self, title: str = "Cuprins"):
        """Add a Word-native TOC using field codes. Updates via F9 in Word."""
        # TOC heading
        ...
        instr_run.addnext(sep_run)

        # Placeholder text
        placeholder_run = OxmlElement("w:r")
        placeholder_text = OxmlElement("w:t")
        placeholder_text.text = "[Faceți clic dreapta și selectați 'Actualizare câmp' pentru a genera cuprinsul]"
        placeholder_run.append(placeholder_text)
        sep_run.addnext(placeholder_run)
        ...
        placeholder_run.addnext(end_run)
    ...
    def add_footnote(self, paragraph, footnote_text: str) -> int:
        ...
        Creates a w:footnoteReference in the paragraph and a w:footnote
        element in footnotes.xml with proper formatting.
        ...
        # Run 1: Footnote reference number (superscript)
        ...
        fn_r1_ref = etree.SubElement(fn_r1, qn("w:footnoteRef"))
        ...
        # Add footnote reference in the main document paragraph
        p_element = paragraph._p

        # Create run with footnoteReference
    ...
    def inject_footnotes_from_citations(self, paragraph, text: str, citation_formatter) -> str:
        """Find citation markers in text, convert to footnotes, return cleaned text.
        ...
        - (Author et al., Year)
        """
        pattern = r'\(([A-ZÀ-Ž][a-zà-ž]+(?:\s(?:și|and|&|et)\s[A-ZÀ-Ž][a-zà-ž]+)?(?:\s+et\s+al\.)?),\s*(\d{4})(?:,\s*(?:pp?\.\s*)([\d\-–]+))?\)'
        ...
        for match in reversed(matches):
            ...
            year = match.group(2)
            pages = match.group(3) if match.group(3) else None
            fn_text = citation_formatter(author, year, pages)
            footnotes_to_add.insert(0, fn_text)
            clean_text = clean_text[:match.start()] + clean_text[match.end():]

        # Add paragraph with cleaned text, then attach footnotes
        for fn_text in footnotes_to_add:
            self.add_footnote(paragraph, fn_text)
